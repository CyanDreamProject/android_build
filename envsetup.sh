...
- cdgerrit: A Git wrapper that fetches/pushes patch from/to CM Gerrit Review.
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cd_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $CD_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       CD_BUILD=
    fi
    export CD_BUILD
...
{
    target=$1
    CD_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/cyandream/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var CD_VERSION)
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$CD_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $CD_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/cyandream/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
